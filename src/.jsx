...
function newPersonaDraft(name = "New Persona", thumbnail = "") {
  return {
    ...
    favorites: {
      food: [], movies: [], tvShows: [], music: [],
      books: [], vacations: [], games: [], sports: []
    },
    notes: [],
    archetype: "",
    ethnicity: "",
...
function Dashboard({ team, personas, threads, following, manualUsers, onOpenPersonaPeek }){
  ...
  const myMsgs = messagesSentByMember[actor] || 0;
...
function AdminSafe({personas, setPersonas, goTab, archivedPersonas, archivePersonaById, unarchivePersonaById, team, setTeam}) {
  const [archiveId, setArchiveId] = useState(personas?.[0]?.id || "");
  const [unarchiveId, setUnarchiveId] = useState(archivedPersonas?.[0]?.id || "");
  useEffect(() => { setArchiveId(personas?.[0]?.id || ""); }, [personas]);
  useEffect(() => { setUnarchiveId(archivedPersonas?.[0]?.id || ""); }, [archivedPersonas]);

  const [name, setName] = React.useState("");
  const [thumbnail, setThumbnail] = React.useState("");
  const fileRef = React.useRef(null);

  const pickFile = ()=>{ try{ fileRef.current?.click(); }catch{} };
  const onFile = (e)=>{
    try{
      const f = e?.target?.files?.[0]; if(!f) return;
      const r = new FileReader();
      r.onload = ()=> setThumbnail(String(r.result||""));
      r.readAsDataURL(f);
      e.target.value = "";
  ...
  const create = ()=>{
    ...
    const th = String(thumbnail||"");
    if(!nm || !th){ alert("Provide name & thumbnail"); return; }
    const p = newPersonaDraft(nm, th);
    try{ setPersonas && setPersonas(prev => (Array.isArray(prev)? [...prev,p] : [p])); }catch(e){ console.error("setPersonas failed", e); }
    try{ queueMicrotask(()=>{ try{ goTab && goTab('personas'); }catch(e){ console.error('goTab failed', e); } }); }catch{}
    setName(""); setThumbnail("");
  };

  const small = { fontSize:12, color:"#6b7280" };
  const input = { width:"100%", boxSizing:"border-box", padding:"10px 12px", border:"1px solid #e5e7eb", borderRadius:8, fontSize:14, background:"#fff" };
    ...
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
      <div style={card}>
        ...
        <div style={{ display:"grid", gap:12 }}>
          <div>
            <div style={small}>Archive an active persona</div>
            <div style={{ display:"flex", gap:8 }}>
              <select id="archive-select" aria-label="Select active persona to archive" title="Select active persona to archive" style={{ ...input, flex:1 }} value={archiveId} onChange={e=>setArchiveId(e.target.value)}>
                {(Array.isArray(personas)?personas:[]).map(p => (
                  <option key={p.id} value={p.id}>{p.name}</option>
                ))}
              </select>
              <button style={btn} title="Archive selected persona" onClick={()=>{
                const id = String(archiveId||"");
                if(id) archivePersonaById(id);
          ...
          <div>
            ...
            <div style={{ display:"flex", gap:8 }}>
              <select id="unarchive-select" aria-label="Select archived persona to restore" title="Select archived persona to restore" style={{ ...input, flex:1 }} value={unarchiveId} onChange={e=>setUnarchiveId(e.target.value)}>
                {(Array.isArray(archivedPersonas)?archivedPersonas:[]).map(p => (
                  <option key={p.id} value={p.id}>{p.name}</option>
                ))}
              </select>
              <button style={subtle} title="Unarchive selected persona" onClick={()=>{
                const id = String(unarchiveId||"");
                if(id) unarchivePersonaById(id);
              }} type="button">Unarchive</button>
            </div>
            <div style={{ ...small, marginTop:6 }}>Archived count: {(archivedPersonas||[]).length}</div>
          </div>
        </div>
        
        <div style={{ fontWeight:700, marginBottom:6 }}>Create Persona</div>
        <div style={{ display:"grid", gridTemplateColumns:"96px 1fr", gap:12, alignItems:"center" }}>
          <div style={{ width:84, height:84, borderRadius:12, background:"#f3f4f6", border:"1px solid #e5e7eb", overflow:"hidden", display:"grid", placeItems:"center" }}>
            {thumbnail ? <img src={thumbnail} alt={name||"preview"} style={{ width:"100%", height:"100%", objectFit:"cover" }} /> : <div style={{ color:"#6b7280", fontWeight:700 }}>img</div>}
          </div>
          <div style={{ display:"grid", gap:8 }}>
            <div>
              <div style={small}>Persona name (required)</div>
              <input value={name} onChange={e=>setName(e.target.value)} placeholder="Raven Rhynne" style={input} />
            </div>
            <div style={{ display:"flex", gap:8 }}>
              <input type="file" accept="image/*" ref={fileRef} onChange={onFile} style={{ display:"none" }} />
              <button style={subtle} onClick={pickFile}>Upload thumbnail</button>
              <button style={btn} onClick={create} disabled={!name.trim() || !thumbnail}>Create</button>
            </div>
          </div>
        </div>
      </div>

      <AdminTeamSafe team={team} setTeam={setTeam} />
    </div>
  );
}

// Localized error boundary for individual persona cards
...
function unlinkLink(user, id){
  ...
  const same = (a,b)=> norm(a.platform)===norm(b.platform) && norm(a.username)===norm(b.username);
  const prev = getLinked(user) || [];
  const next = prev.filter(x => !same(x, id));
  try { localStorage.setItem(key, JSON.stringify(next)); } catch {}
  // Also remove from dismissed list so it can reappear in Potential matches
  try {
    const dk = "dismissed_" + user.id;
    const darr = JSON.parse(localStorage.getItem(dk) || "[]");
    const cleaned = (Array.isArray(darr)?darr:[]).filter(x => !same(x, id));
    if (cleaned.length !== (Array.isArray(darr)?darr.length:0)) localStorage.setItem(dk, JSON.stringify(cleaned));
  } catch {}
  try { setLinksNonce(n=>n+1); } catch {}
  try { logDomain("user_link_removed", { userId:user.id, platform:id.platform, username:id.username }); } catch {}
}
...
<TabBoundary>
        <div style={{ display:"grid", gap:16, gridTemplateColumns:"repeat(auto-fill, minmax(380px, 1fr))" }}>
          {personas.map((p,i) => <CardBoundary key={p.id || i} label={`Persona ${p?.name||(''+i)}`}>
            <div key={p.id} id={`persona-card-${p.id}`} style={{ ...cardStyle, outline: (hoverIndex===i && dragType==='persona') ? "2px dashed #3b82f6" : "none" }} draggable onDragStart={()=>dragStart('persona', i)} onDragOver={onDragOverCard} onDrop={()=>onDropCard('persona', i)} onDragEnter={()=>setHoverIndex(i)} onDragLeave={()=>setHoverIndex(-1)}>
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
                ...
                <div style={{ display:"flex", gap:6 }}>
                  ...
                  {p.thumbnail && <button style={subtleBtn} onClick={()=>{ const c=[...personas]; c[i].thumbnail=""; setPersonas(c); try{ log("persona_photo_remove",{ persona: p?.name || personas[i]?.name || "", who: you?.name || ownerName }); }catch(_){ } }}>Remove</button>}
                </div>
              </div>
...
<div style={{ display:"grid", rowGap:8, marginBottom:12 }}>
  ...
  <div key={i} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"6px 0" }}>
    ...
    <button type="button" onClick={(e)=>{ e.preventDefault(); e.stopPropagation(); (()=>unlinkLink(selUser, lnk))(e); }} style={{ padding:"6px 10px", border:"1px solid #e5e7eb", borderRadius:8, background:"#fff", cursor:"pointer" }}>Unlink</button>
  </div>
))}
    ...
    <div style={{ display:"grid", gap:6 }}>
      {(linked||[]).length ? (linked||[]).map(id => (
        <div key={id.platform+':'+id.handle} style={{ display:"flex", gap:8, alignItems:"center" }}>
          ...
          <button type="button" onClick={(e)=>{ e.preventDefault(); e.stopPropagation(); (()=>onUnlink && onUnlink(id))(e); }} style={{ border:"1px solid #e5e7eb", borderRadius:8, padding:"4px 8px", background:"#fff", cursor:"pointer" }}>Unlink</button>
        </div>

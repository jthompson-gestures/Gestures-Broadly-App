diff --git a/App_Code_1_8_full.txt b/App_Code_1_8_full.txt
new file mode 100644
index 0000000000000000000000000000000000000000..8ef87cf77129e738047985578875ffb3105bad82
--- /dev/null
+++ b/App_Code_1_8_full.txt
@@ -0,0 +1,6845 @@
+
...
+const personaIdAliases = new Map();     // legacy_id -> current_id
+
+function resolvePersonaId(rawId){
+  const original = String(rawId ?? "").trim();
+  if (!original) return original;
+  let cur = original;
+  const seen = new Set();
+  while (personaIdAliases.has(cur) && !seen.has(cur)) {
+    seen.add(cur);
+    const next = String(personaIdAliases.get(cur) ?? "").trim();
+    if (!next) break;
+    cur = next;
+  }
+  return cur || original;
+}
+
+function personaIdCandidates(rawId){
+  const original = String(rawId ?? "").trim();
+  const canonical = resolvePersonaId(original);
+  const related = new Set();
+
+  if (original) related.add(original);
+  if (canonical) related.add(canonical);
+
+  const target = canonical || original;
+  if (target) {
+    for (const [legacy, aliasTarget] of personaIdAliases.entries()) {
+      const legacyId = String(legacy ?? "").trim();
+      if (!legacyId) continue;
+      const normalizedTarget = resolvePersonaId(aliasTarget);
+      if (normalizedTarget && normalizedTarget === target) {
+        related.add(legacyId);
+        const nextTarget = String(aliasTarget ?? "").trim();
+        if (nextTarget) related.add(nextTarget);
+      }
+    }
+  }
+
...
+}
+
+function personaMatchesId(persona, ids){
+  if (!persona || !Array.isArray(ids) || !ids.length) return false;
+  const candidates = personaIdCandidates(persona?.id);
+  if (!candidates.length) return false;
+  const wanted = new Set(ids.filter(Boolean).map(id => String(id).trim()));
+  if (!wanted.size) return false;
+  return candidates.some(pid => pid && wanted.has(pid));
+}
+
+function registerPersonaAlias(fromId, toId){
+  const from = String(fromId ?? "").trim();
+  const to = String(toId ?? "").trim();
+  if (!from || !to || from === to) return;
+  // Collapse chains so any historical id resolves to the latest canonical id.
+  let canonical = to;
+  const seen = new Set([from]);
+  while (personaIdAliases.has(canonical) && !seen.has(canonical)) {
+    seen.add(canonical);
+    canonical = String(personaIdAliases.get(canonical) ?? canonical).trim() || canonical;
+  }
+  personaIdAliases.set(from, canonical);
+}
+let __favSyncStarted = false;
+let __favPersonasRef = null;
+let __favSetPersonas = null;
+
+function promotePersonaId(oldId, newId){
+  const from = String(oldId || "").trim();
+  const to = String(newId || "").trim();
+  if (!from || !to || from === to) return;
+
+  registerPersonaAlias(from, to);
+  const target = resolvePersonaId(to);
+
+  if (favoritesHydratedSet.has(from)) {
+    favoritesHydratedSet.delete(from);
+    favoritesHydratedSet.add(target);
+  }
+
+  if (favoritesCacheMap.has(from)) {
+    const snapshot = favoritesCacheMap.get(from);
+    favoritesCacheMap.delete(from);
+    favoritesCacheMap.set(target, snapshot);
+  }
+
+  if (Array.isArray(__favPersonasRef)) {
+    __favPersonasRef = __favPersonasRef.map(p => {
+      return String(p?.id || "").trim() === from ? { ...p, id: target } : p;
+    });
+  }
+
+  if (typeof __favSetPersonas === 'function') {
+    __favSetPersonas(prev => {
+      const list = Array.isArray(prev) ? prev.slice() : [];
+      const idx = list.findIndex(p => String(p?.id || "").trim() === from);
+      if (idx === -1) return prev;
+      const next = list.slice();
+      next[idx] = { ...next[idx], id: target };
+      return next;
+    });
+  }
+}
...
+  try {
+    const curId = String(persona?.id || "").trim();
...
+    const freshId = String(data?.id || '').trim();
+    if (!freshId) return null;
+    promotePersonaId(curId, freshId);
+    return freshId;
+  } catch (e) {
...
+  const canonicalId = resolvePersonaId(persona_id);
+  if (!canonicalId || favoritesHydratedSet.has(canonicalId)) return;
+  try {
+    const { data, error } = await supabase
...
+    const idx = personas.findIndex(p => String(p.id) === canonicalId);
+    if (idx === -1) return;
+
+    const copy = [...personas];
+    let p = copy[idx];
...
+    setPersonas(copy);
+
+    favoritesHydratedSet.add(canonicalId);
+    favoritesCacheMap.set(canonicalId, (data||[]).map(r => ({ category: canonCategory(r.category), value: r.value })));
+    console.log('[favorites] hydrated from DB:', canonicalId, favoritesCacheMap.get(canonicalId));
+  } catch (e) {
...
+  const persona_id = resolvePersonaId(__ensured || persona?.id);
+
+
...
+      <AiBehaviorControls persona={p} onChange={(next)=> updatePersona(p.id, before => ({ ...before, aiBehavior: next }))} />
+Affiliate Marketing */}
...
+    const target = resolvePersonaId(id);
+    setPersonas(prev => {
+      const list = Array.isArray(prev) ? prev : [];
+      let changed = false;
+      const next = list.map(p => {
+        if (String(p?.id) === target || String(p?.id) === String(id)) {
+          changed = true;
+          return { ...p, thumbnail: replaceThumbData };
+        }
+        return p;
+      });
...
+      return ({
+        ...p,
+        id: (p && p.id != null && String(p.id).trim()) ? String(p.id).trim() : uid(),
...
+    const target = resolvePersonaId(id);
+    try{ logDomain("persona_archive_requested",{ id:String(target||id) }); }catch{};
+    setPersonas(prev => {
+      const list = Array.isArray(prev) ? prev : [];
+      const idx = list.findIndex(p => String(p?.id) === target || String(p?.id) === String(id));
+      if (idx === -1) { try{ logDomain("persona_archive_failed",{ id:String(target||id) }); }catch{}; return prev; }
...
+      try{ logDomain("persona_archive_succeeded",{ id:String(target||id), persona: moved?.name||"" }); }catch{};
...
+    const target = resolvePersonaId(id);
+    try{ logDomain("persona_unarchive_requested",{ id:String(target||id) }); }catch{};
+    setArchivedPersonas(prev => {
+      const list = Array.isArray(prev) ? prev : [];
+      const idx = list.findIndex(p => String(p?.id) === target || String(p?.id) === String(id));
+      if (idx === -1) { try{ logDomain("persona_unarchive_failed",{ id:String(target||id) }); }catch{}; return prev; }
+      const movedRaw = list[idx];
+      const moved = typeof coercePersona === "function" ? coercePersona(movedRaw) : movedRaw;
+      // push back into active personas
+      setPersonas(pPrev => dedupeById([...(Array.isArray(pPrev)?pPrev:[]), moved]));
+      try{ logDomain("persona_unarchive_succeeded",{ id:String(target||id), persona: moved?.name||"" }); }catch{};
+      const next = list.slice(0, idx).concat(list.slice(idx+1));
+      return next;
+    });
+  }
...
+    const candidateIds = personaIdCandidates(id);
+    const canonicalId = resolvePersonaId(id);
+    const logId = canonicalId || candidateIds[0] || "";
+    setPersonas(prev => {
+      const list = Array.isArray(prev) ? prev : [];
+      let idx = -1;
+      for (const candidate of candidateIds) {
+        if (!candidate) continue;
+        idx = list.findIndex(p => String(p.id) === candidate);
+        if (idx !== -1) break;
+      }
+      if (idx === -1) return prev;
+      const before = list[idx];
+      let after = typeof patch === "function" ? patch(before) : { ...before, ...patch };
+      if (!after || typeof after !== "object") return prev;
+      if (after === before) return prev;
+
+      const afterId = String(after.id ?? "").trim();
+      const afterCandidates = personaIdCandidates(afterId);
+      const matchesTarget = afterCandidates.some(pid => pid && candidateIds.includes(pid));
+      if (!matchesTarget) {
+        after = { ...after, id: before.id };
+      }
+
+      if (after.id !== before.id) {
+        registerPersonaAlias(before.id, after.id);
+      }
+
+      const next = list.slice();
+      next[idx] = after;
...
+        logPersonaEvent("persona_updated", { id: logId || before?.id || "", name: after?.name || before?.name || "", fields, changes }, actorName);
+      }
+      return next;
+    });
+  }
+  function lockBio(id){
+    const candidates = personaIdCandidates(id);
+    setPersonas(prev => {
+      if (!Array.isArray(prev)) return prev;
+      let changed = false;
+      const next = prev.map(p => {
+        if (personaMatchesId(p, candidates)) {
+          changed = true;
+          return { ...p, bioSaved:true, bioSavedAt: Date.now() };
+        }
+        return p;
+      });
+      return changed ? next : prev;
...
+            ...p,
+            affiliateMarketing: {
+              ...(p.affiliateMarketing||{text:'',locked:false,savedAt:0}),
+              locked:true,
+              savedAt: Date.now()
+            }
+          };
...
+    const candidates = personaIdCandidates(id);
+    const canonical = resolvePersonaId(id);
+    setPersonas(prev => {
+      const list = Array.isArray(prev) ? prev : [];
+      const victim = list.find(p => personaMatchesId(p, candidates));
+      const next = list.filter(p => !personaMatchesId(p, candidates));
+      if (victim) { try { logPersonaEvent("persona_deleted", { id:String(canonical || candidates[0] || id || ""), name: victim?.name||"" }, actorName); } catch {} }
+      return next;
+    });
+  }
...
+      const el = filePickers.current[key];
+      if (el) { el.click(); return; }
+    }
+    const fallback = filePickers.current[String(id ?? "")];
+    if (fallback) fallback.click();
+  };
+  const onFileChange=(id,e)=>{ const f=e.target.files?.[0]; if(!f) return; const r=new FileReader(); r.onload=()=>{ const data=r.result; updatePersona(id, before => { try{ logDomain("persona_photo_upload",{ persona: before?.name||"", id: before?.id }, data); }catch{} return { ...before, thumbnail: data }; }); }; r.readAsDataURL(f); e.target.value=""; };
+
+  const voicePickers = useRef({});
+  const triggerVoice = (id)=>{
+    const candidates = personaIdCandidates(id);
+    for (const key of candidates) {
+      const el = voicePickers.current[key];
+      if (el) { el.click(); return; }
+    }
+    const fallback = voicePickers.current[String(id ?? "")];
+    if (fallback) fallback.click();
+  };
+  const onVoiceChange=(id,e)=>{ const f=e.target.files?.[0]; if(!f) return; const r=new FileReader(); r.onload=()=>{ const data=r.result; updatePersona(id, before => { try{ logDomain("persona_voice_upload",{ persona: before?.name||"", id: before?.id }); }catch{} return { ...before, voice: data }; }); }; r.readAsDataURL(f); e.target.value=""; };
+
+  const initials=(name="")=> name.split(" ").filter(Boolean).map(s=>s[0]).join("").slice(0,2).toUpperCase();
...
+    style={inputStyle}
+    value={p.birthday || ""}
+    onChange={(e)=> updatePersona(p.id, { birthday: e.target.value })}
+  />
...
+                          style={subtleBtn}
+                          onClick={()=>{
+                            const d = handleDraft[p.id] || { username:"", platform: CHANNEL_CHOICES[0] };
+                            if(!d.username.trim()) return;
+                            updatePersona(p.id, before => {
+                              const arr = Array.isArray(before.handles) ? before.handles.slice() : [];
+                              arr.push({ platform: d.platform, username: d.username.trim() });
+                              return { ...before, handles: arr };
+                            });
+                            setHandleDraft({ ...handleDraft, [p.id]: { ...d, username: "" } });
+                          }}
+                        >Add</button>
+                      </>
+                    ); })()}
...
+                              updatePersona(p.id, before => {
+                                const arr = Array.isArray(before.handles) ? before.handles.slice() : [];
+                                const removed = arr.splice(hi,1)[0];
+                                try{ log("persona_handle_remove",{ persona: before?.name || "", platform: removed?.platform, username: removed?.username, who: you?.name || ownerName }); }catch(_){ }
+                                return { ...before, handles: arr };
+                              });
+                            }}
+                            style={{ background:"none", border:"none", color:"#ef4444", marginLeft:6 }}
+                          >x</button>
+                        </span>
+                      ))}
+                    </div>
...
+                    value={bioTextOf(p)}
+                    onChange={e=> updatePersona(p.id, { bio: e.target.value })}
...
+                      accept="audio/*"
+                      ref={el=> {
+                        if (el) voicePickers.current[p.id] = el;
+                        else delete voicePickers.current[p.id];
+                      }}
+                      onChange={e=>onVoiceChange(p.id,e)}
+                      style={{ display:'none' }}
+                    />
+                    {p.voice ? (
+                      <>
+                        <audio controls src={p.voice} style={{ flex:1, minWidth:220 }} />
+                        <button style={subtleBtn} onClick={()=>triggerVoice(p.id)}>Upload</button>
+                        <button style={subtleBtn} onClick={()=> updatePersona(p.id, prev => ({ ...prev, voice: "" }))}>Remove</button>
+                      </>
+                    ) : (
+                      <>
+                        <button style={subtleBtn} onClick={()=>triggerVoice(p.id)}>Upload</button>
...
+                <ChipField label="Affection Cues" items={p.affectionCues||[]} placeholder="e.g., nicknames, playful teasing" onChange={arr=> updatePersona(p.id, before => ({ ...before, affectionCues: arr }))} />
+                <ChipField label="Signature Phrases" items={p.signature||[]} placeholder="Catchphrases, sign-offs" onChange={arr=> updatePersona(p.id, before => ({ ...before, signature: arr }))} />
+                <ChipField label="Emojis" items={p.emojis||[]} placeholder="e.g., 😊, 🔥, ✨" onChange={arr=> updatePersona(p.id, before => ({ ...before, emojis: arr }))} />
+                <ChipField label="Interests" items={p.interests||[]} placeholder="Hobbies / topics" onChange={arr=> updatePersona(p.id, before => ({ ...before, interests: arr }))} />
+
...
+      wiki
+      label="Food"
+      items={p.favorites?.food||[]}
+      placeholder="e.g., sushi, tacos"
+      onChange={arr=>{
+        updatePersona(p.id, before => {
+          const fav = { ...(before.favorites||{}) };
+          fav.food = arr;
+          return { ...before, favorites: fav };
+        });
+      }}
+    />
+    <ChipField
...
+      wiki
+      label="TV Shows"
+      items={p.favorites?.tvShows||[]}
+      placeholder="e.g., sitcoms"
+      onChange={arr=>{
+        updatePersona(p.id, before => {
+          const fav = { ...(before.favorites||{}) };
+          fav.tvShows = arr;
...
+      wiki
+      label="Music — Artists"
+      items={p.favorites?.music?.artists||[]}
+      placeholder="e.g., Daft Punk"
+      onChange={arr=>{
+        updatePersona(p.id, before => {
+          const fav = { ...(before.favorites||{}) };
+          const music = (fav.music && typeof fav.music === 'object') ? { ...fav.music } : { artists:[], genres:[] };
+          music.artists = arr;
+          fav.music = music;
+          return { ...before, favorites: fav };
+        });
...
+      wiki
+      label="Music — Genres"
+      items={p.favorites?.music?.genres||[]}
...
+      wiki
+      label="Books — Titles"
+      items={p.favorites?.books?.titles||[]}
+      placeholder="e.g., Dune"
+      onChange={arr=>{
+        updatePersona(p.id, before => {
+          const fav = { ...(before.favorites||{}) };
+          const books = (fav.books && typeof fav.books === 'object') ? { ...fav.books } : { titles:[], authors:[] };
+          books.titles = arr;
+          fav.books = books;
+          return { ...before, favorites: fav };
+        });
...
+      wiki
+      label="Books — Authors"
+      items={p.favorites?.books?.authors||[]}
...
+      wiki
+      label="Games"
+      items={p.favorites?.games||[]}
+      placeholder="e.g., RPGs"
+      onChange={arr=>{
+        updatePersona(p.id, before => {
+          const fav = { ...(before.favorites||{}) };
+          fav.games = arr;
...
+    <ChipField
+      wiki
+      label="Types of Vacations"
+      items={p.favorites?.vacations||[]}
...
+    <ChipField
+      wiki
+      label="Vacation Locations"
+      items={p.favorites?.vacationLocations||[]}
+      placeholder="e.g., Paris, Bali"
+      onChange={arr=>{
+        updatePersona(p.id, before => {
+          const fav = { ...(before.favorites||{}) };
+          fav.vacationLocations = arr;
+          return { ...before, favorites: fav };
+        });
+      }}
+    />
...
+
+
+                <ChipField label="Style" items={p.style||[]} placeholder="Wardrobe / vibe" onChange={arr=> updatePersona(p.id, before => ({ ...before, style: arr }))} />
+                <ChipField label="Values" items={p.values||[]} placeholder="Core principles" onChange={arr=> updatePersona(p.id, before => ({ ...before, values: arr }))} />
+                <ChipField label="Do's & Don'ts" items={p.dosDonts||[]} placeholder="Guidelines / avoidances" onChange={arr=> updatePersona(p.id, before => ({ ...before, dosDonts: arr }))} />
+                <ChipField label="Goals" items={p.goals||[]} placeholder="Short/long-term" onChange={arr=> updatePersona(p.id, before => ({ ...before, goals: arr }))} />
+                <ChipField label="Fears" items={p.fears||[]} placeholder="What they avoid" onChange={arr=> updatePersona(p.id, before => ({ ...before, fears: arr }))} />
+                <ChipField label="Boundaries" items={p.boundaries||[]} placeholder="Hard no's" onChange={arr=> updatePersona(p.id, before => ({ ...before, boundaries: arr }))} />
+                <ChipField label="Tone" items={p.tone||[]} definitions={TONE_DEFS} options={COMMON_TONES} placeholder="e.g., warm & friendly" onChange={arr=> updatePersona(p.id, before => ({ ...before, tone: arr }))} />
...
+                    onChange={e=> updatePersona(p.id, before => ({
+                      ...before,
+                      affiliateMarketing: {
+                        ...(before.affiliateMarketing||{ text:"", locked:false, savedAt:0 }),
+                        text: e.target.value
+                      }
+                    }))}
